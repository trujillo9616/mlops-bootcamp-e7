...
def elbow_model(config_path: Text) -> None:
    ...
    for k in range(3, 7):
        ...
        Scores["Davies_Bouldin_Score"].append(
            davies_bouldin_score(data_transformed, kmeans.labels_)
        )
        Scores["Calinski_Harabasz_Score"].append(
            calinski_harabasz_score(data_transformed, kmeans.labels_)
        )
        Scores["Silhouette_Score"].append(
            silhouette_score(data_transformed, kmeans.labels_)
        )
